...
bool FileUtil::readFileToVector(vector<vector<string>>& a,string ts) {
	ifstream csvInput(ts);
	if (!csvInput)
	{
		csvInput.close();
		return false;
...
bool FileUtil::readConfig(unordered_map<string, int>& config,string cf) {
	ifstream iniInput(cf);
	if (!iniInput)
	{
		iniInput.close();
		return false;
